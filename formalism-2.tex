...
\end{array}\]
Notice that it is only partially defined for when the
...
which we tour from an implementator's perspective,
starting with the input edit state
...
Now the piece $\shardlit{(}$ to the right of the cursor is atomic,
...
the parentheses shards into a tile frame.
