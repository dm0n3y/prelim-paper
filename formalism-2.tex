...
\end{array}\]
Note that it is only partially defined for when the
...
We will start by introducing the central \emph{parsability} invariant
that governs action behavior,
then describe the eight rules defining the action judgment, grouped
into five feature categories:
moving, % (\ref{sec:moving}),
constructing, % (\ref{sec:constructing}),
selecting, % (\ref{sec:selecting}),
restructuring, and % (\ref{sec:restructuring}), and
deleting. % (\ref{sec:deleting}).

\subsubsection{Parsability} \label{sec:parsability}

\input{term-flattening}

A segment $C$ is called \emph{$s$-parsable} if there
exists a term $x$ of sort $s$ that \emph{flattens} to
it, as defined by the term flattening judgments $\flattenTerm{x}{C}$
...
Actions are governed by the following theorem.
...
\subsubsection{Constructing} \label{sec:constructing}
\[
  \inferrule[Construct]{
...
\subsubsection{Deleting} \label{sec:deleting}
\[
  \inferrule[Delete]{
