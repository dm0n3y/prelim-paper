...
\subsection{Inserting and removing} \label{sec:inserting-removing}
...
A selection containing no shards is called \emph{intact}, otherwise
\emph{cracked}.
...
since you may insert a selection of whole tiles anywhere
...
Our remaining task is to restructure our code so that \code{x}
...
The \code{in}-shard in our backpack cannot be assembled into
a whole tile on its own, meaning at least one matching shard---in
this case, \code{let x , y =}---is
anchored within the current tile sequence.
This restricts our movement of the \texttt{in}
