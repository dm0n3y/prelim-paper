...
\subsection{Selecting and restructuring tiles}
...
\tylr~ recovers much of this flexibility, letting you
make arbitrary range selections up to token boundaries
and manipulate them in a structure-preserving manner.
...
and select the tiles you just inserted in the $y$-coordinate,
an impossible selection in other structure editors:
...
\end{minipage}
You then pick it up into your \emph{backpack} to enter
\emph{restructuring mode}.
...
Your backpack is a structure-aware clipboard
accompanying your cursor that ensures you
put down its contents at reasonable cursor positions,
\ie, the result has a well-formed tile structure
after hole fixing.
In the last example, because your selection consisted of
whole expression tiles, any expression position would
have been a valid restructuring target.
Most selections, however, do not consist solely of whole
tiles, in which case there are additional restrictions.

For example, say you would like the circles' radii to grow
...
Your goal now is to restructure your code so that \texttt{x}
and \texttt{y} are bound to the origin coordinates
\texttt{( n * n / 9 , n )} in the body of the let term.
You begin by selecting the \texttt{in} delimiter,
which disassembles the \texttt{let}-tile into a pair of \emph{shards}.
You then move right, once, and put down the \texttt{in}
shard, upon which \tylr~ reassembles the \texttt{let}-tile
...
Notice that \tylr~ skipped all cursor positions within
the parentheses tile when you moved right.
\tylr~ recognizes that the \texttt{in} shard in the
backpack has a matching shard anchored within the current
tile sequence, so it restricts your movement to cursor
positions within the same sequence, as these are the only valid
targets that preserve proper tile nesting
on reassembly.
