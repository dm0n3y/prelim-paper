...
Shown above, the initial implementation draws unit circles
along the line $y = x$.
...
shows the top-level function expression binding the index \code{n}
and returning an indexed circle.
Notice how every term has a convex hexagonal shape.

Highlighted within each term is a substructure called a \emph{tile}.
Each term's highlighted tile encompasses all root-level tokens used to
represent the term, along with children of the term that the tokens
delimit on both sides---we call such children \emph{bidelimited}.
...
as well as the bidelimited pattern child, but does not
extend to the \emph{unidelimited} body.
...
Tiles enjoy a more linear structure compared to the
strictly hierarchical terms.
Notice in the first edit state of Figure \ref{fig:pan-term-view},
for example, how the tiles comprising the function body are siblings
with the function term's root tile.

\note{talk about tips and well-formedness}
