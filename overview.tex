...
in a rush to end its hunger, which can happen in one of two ways.
...
Finally, we examine how to remove tiles in \tylr.

As with selection, term-based structure editors
limit delete operations so they can only act on
complete terms, which forces awkward and preemptive
...
to save the children first by moving them elsewhere
before you can actually delete the node.

Meanwhile, it can be difficult to predict the
effects of term-based deletion in text-like
structure editors, such as JetBrains MPS, that
provide less visual indication of the term
structure.
The impact is a delete operation can be much
more destructive than expected, \eg,
pressing Backspace on an individual token
causes a large range to disappear.
Indeed, a (controlled) user study of MPS revealed
that even \emph{experts} feel that deletion in MPS
is relatively inaccurate (compared to similar
subjective assessments made by text editor users),
which we interpret to mean that they have trouble
predicting the effects of delete operations.


Tile-based editing reduces the destructive effect of deletions
compared to term-based editing; moreover \tylr~ explicitly
highlights all code that will be removed upon your deletion
so that you never have to guess.
