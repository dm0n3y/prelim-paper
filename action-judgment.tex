\begin{figure*}
  \judgbox{\performAction{\editState_1}{\action}{\editState_2}}{$\editState_1$ transitions via $\alpha$ to $\editState_2$}
  \begin{mathpar}
  \inferrule[MoveRightEnterTile]{
    \disassembleTile{\tile^s} = \token^{s}\selection\\
    parseZipper(
      \zipper{\pointing{\tiles^s_1\token^{s}~}{~\selection\tiles^s_2}}{\tfrelem^s}
    ) = \zeta^{s'}
  }{
    \performAction{
      \zipper{\pointing{\tiles^s_1}{\tile^s\tiles^s_2}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zeta^{s'}
    }
  } \ \ \ \ \ \
  \inferrule[MoveRightExitTile]{
    \disassembleTileFrame{\tfrelem^s} =
    \zipper{\pointing{\selection_1}{\token^{s'}\selection_2}}{\tfrelem^{s'}}\\
    parseZipper(
      \zipper{\pointing{\selection_1\tiles^s\token^{s'}}{\selection_2}}{\tfrelem^{s'}}
    ) = \zeta^{s''}
  }{
    \performAction{
      \zipper{\pointing{\tiles^s}{\cdot}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zeta^{s''}
    }
  } \\
  \inferrule[ConstructTile]{
    \disassembleTile{\tile^s} = \token^s\selection_3 \\
    \fixHolesSelections{\rtipconc{s}}{\selection_1\token^s}{\selection_3\selection_2}{\ltipconc{s}}{\selection_4}{\selection_5} \\\\
    parseZipper(
      \zipper{\pointing{\selection_4}{\selection_5}}{\tfrelem^{s}}
    ) = \zeta^{s'}
  }{
    \performAction{
      \zipper{\pointing{\selection_1}{\selection_2}}{\tfrelem^s}
    }{
      \actionlit{construct}\ \tile^s
    }{
      \zeta^{s'}
    }
  } \ \ \ \ \ \ \ \
  \inferrule[MarkPointing]{
  }{
    \performAction{
      \zipper{\pointing{\selection_1}{\selection_2}}{\tfrelem^s}
    }{
      \actionlit{mark}
    }{
      \zipper{\selecting{\selection_1}{\strut~\cdot~}{\selection_2}{\tfrelem^s}}
    }
  } \\
  \inferrule[SelectLeftToken]{
    \parseSelection{\token^s\selection_2} = \selection'_2
  }{
    \performAction{
      \zipper{\selecting{\selection_1\token^s}{\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{left}
    }{
      \zipper{\selecting{\selection_1}{\selection'_2}{\selection_3}}{\tfrelem^s}
    }
  } \ \ \
  \inferrule[SelectLeftTile]{
    \disassembleTile{\tile^s} = \selection_4\token^s \\
    \parseSelection{\token^s\selection_2} = \selection'_2
  }{
    \performAction{
      \zipper{\selecting{\selection_1\tile^s}{\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{left}
    }{
      \zipper{\selecting{\selection_1\selection_4}{\selection'_2}{\selection_3}}{\tfrelem^s}
    }
  } \ \ \
  \inferrule[SelectLeftExit]{
    \disassembleTileFrame{\tfrelem^s} =
    \zipper{\pointing{\selection_3\token^{s'}}{\selection_4}}{\tfrelem^{s'}}
  }{
    \performAction{
      \zipper{\selecting{\cdot}{\selection_1}{\selection_2}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{left}
    }{
      \zipper{\selecting{\selection_3}{\selection_1\token^{s'}}{\selection_2\selection_4}}{\tfrelem^{s'}}
    }
  } \\
  \inferrule[SelectRightToken]{
    \parseZipper{\zipper{\pointing{\selection_1\token^s}{\selection_3}}{\tfrelem^s}} =
      \zipper{\pointing{\selection'_1}{\selection'_3}}{\tfrelem^{s'}}
  }{
    \performAction{
      \zipper{\selecting{\selection_1}{\token^s\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zipper{\selecting{\selection'_1}{\selection_2}{\selection'_3}}{\tfrelem^{s'}}
    }
  } \ \ \ \ \ \ \ \
  \inferrule[SelectRightTile]{
    \disassembleTile{\tile^s} = \token^s\selection_4
  }{
    \performAction{
      \zipper{\selecting{\selection_1}{\tile^s\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zipper{\selecting{\selection_1\token^s}{\selection_4\selection_2}{\selection_3}}{\tfrelem^s}
    }
  } \\
  \inferrule[MarkSelecting]{
    \matchesSort{\leftTip{\selection_2}}{\rightTip{\selection_2}} \\
    \fixHolesSelections{\rtipconc{s}}{\selection_1}{\selection_3}{\ltipconc{s}}{\selection'_1}{\selection'_3}
  }{
    \performAction{
      \zipper{\selecting{\selection_1}{\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{mark}
    }{
      \zipper{\restructuring{\selection'_1}{\selection_2}{\selection'_3}}{\tfrelem^s}
    }
  } \\
  \inferrule[MoveRightRestructuring]{
    \text{\note{not\ }}\wholeSelection{s'}{\selection_2}
  }{
    \performAction{
      \zipper{\restructuring{\selection_1}{\selection_2}{\tile^{s'}\selection_3}}{\tfrelem^s}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zipper{\restructuring{\selection_1\tile^{s'}}{\selection_2}{\selection_3}}{\tfrelem^s}
    }
  } \\
  \inferrule[MoveRightRestructuringWholeSame]{
    \wholeSelection{s}{\selection_2} \\
    \stepDisassembleSelection{\tile^s}{\selem\selection_4} \\
    \parseZipper{\zipper{\pointing{\selection_1\selem}{\selection_4\selection_3}}{\tfrelem^s_1}}
      = \zipper{\pointing{\selection_5}{\selection_6}}{\tfrelem^s_2}
  }{
    \performAction{
      \zipper{\restructuring{\selection_1}{\selection_2}{\tile^s\selection_3}}{\tfrelem^s_1}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zipper{\restructuring{\selection_5}{\selection_2}{\selection_6}}{\tfrelem^s_2}
    }
  } \\
  \inferrule[MoveRightRestructuringWholeDifferent]{
    \wholeSelection{s}{\selection_2} \\
    \stepDisassembleSelection{\tile^s}{\selem\selection_4} \\
    \parseZipper{\zipper{\pointing{\selection_1\selem}{\selection_4\selection_3}}{\tfrelem^s_1}}
      = \zipper{\pointing{\selection_5}{\selection_6}}{\tfrelem^{s'}_2} \\
    s\neq s' \\
    \performAction{
      \zipper{\pointing{\selection_5}{\selection_6}}{\tfrelem^{s'}_2}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zip^s
    }
  }{
    \performAction{
      \zipper{\restructuring{\selection_1}{\selection_2}{\tile^s\selection_3}}{\tfrelem^s_1}
    }{
      \actionlit{move}\ \texttt{right}
    }{
      \zip^s
    }
  } \\
  \inferrule[Delete]{
    \fixHolesSelections{\rtipconc{s}}{\filterTiles{s}{\selection_1}}{\filterTiles{s}{\selection_3}}{\ltipconc{s}}{\selection'_1}{\selection'_3}
  }{
    \performAction{
      \zipper{\restructuring{\selection_1}{\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{delete}
    }{
      \zipper{\pointing{\selection'_1}{\selection'_3}}{\tfrelem^s}
    }
  } \\
  \inferrule[MarkRestructuring]{
    \fixHolesSelections{\rtipconc{s}}{\selection_1}{\selection_2\selection_3}{\ltipconc{s}}{\selection_4}{\selection_5} \\
    \parseZipper{\zipper{\pointing{\selection_4}{\selection_5}}{\tfrelem^s}} = \zip^{s'}
  }{
    \performAction{
      \zipper{\restructuring{\selection_1}{\selection_2}{\selection_3}}{\tfrelem^s}
    }{
      \actionlit{mark}
    }{
      \zip^{s'}
    }
  }
  \end{mathpar}

  \vspace{-2px}
  \CaptionLabel{Action performing \note{prevent empty hole tile construction, add justification for constructing tiles rather than tokens}}{fig:perform-action}
  \vspace{-2px}
  \end{figure*}